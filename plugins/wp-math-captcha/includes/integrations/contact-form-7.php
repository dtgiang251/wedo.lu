...
function wpcf7_mathcaptcha_shortcode_handler( $tag ) {
	if ( ! is_user_logged_in() || ( is_user_logged_in() && ! Math_Captcha()->options['general']['hide_for_logged_users'] ) ) {
		...
		if ( !isset( $_POST[$tag->name] ) && !isset( $_POST['_wpcf7'] ) ) {
			...
			return sprintf( ( ( empty( $math_captcha_title ) ) ? '' : $math_captcha_title ) . ' <span class="wpcf7-form-control-wrap %1$s">' . $mc_form[1] . $mc_form[2] . $mc_form[3] . '%3$s</span><input type="hidden" value="' . ( Math_Captcha()->core->session_number - 1 ) . '" name="' . $tag->name . '-sn" /><input type="hidden" value="' . ( $mc_form['result'] ) . '" name="' . $tag->name . '-rs" />', $tag->name, $atts, $validation_error );
		}
	}
}
	...
	function wpcf7_mathcaptcha_validation_filter( $result, $tag ) {
		...
		if ( isset( $_POST[$name] ) && $_POST[$name] !== '' && ! is_admin() ) {
			...
			if ( $session_id !== '' && get_transient( 'cf7_' . $session_id ) !== false ) {
				...
				// if ( absint( get_transient( 'cf7_' . $session_id ) ) != $val ) {
				if ( absint( $_POST[$name . '-rs'] ) != $val ) {
