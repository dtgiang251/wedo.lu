...
class Math_Captcha_Core {
	...
	public function generate_captcha_phrase( $form = '' ) {
		...
		// if( get_transient( $transient_name . '_' . $session_id ) == false  ) {
		
			// var_dump($number[$rnd_input]);
			set_transient( $transient_name . '_' . $session_id, $number[$rnd_input], apply_filters( 'math_captcha_time', Math_Captcha()->options['general']['time'] ) );
		
		// }
		
		$return['result'] = $number[$rnd_input];
		return $return;
	}
