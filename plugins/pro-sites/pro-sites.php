...
function admin_modules() {
	...
	function checkout_output( $content ) {
		...
		if (isset($_POST['bid'])){
			...
			foreach ($blogs_of_user as $id => $obj) {
				...
				if ($permission) {
					...
					if( $is_admin && ! is_pro_site($obj->userblog_id) ) {
		...
		if ( $blog_id && false ) {
			...
			$blog_id = 0;
			if ( $blogs ) {
				$content .= '<h3>' . __( 'Veuillez sélectionner le site à modifier:', 'psts' ) . '</h3>';
				$content .= '<ul>';
				...
				foreach ( $blogs as $blog ) {
					...
					$upgrade_label = is_pro_site( $blog->userblog_id ) ? sprintf( __( 'Modifier "%s"', 'psts' ), $blog->blogname ) : sprintf( __( 'Upgrader "%s"', 'psts' ), $blog->blogname );
					if( empty( $blog_id ) && is_pro_site( $blog->userblog_id ) ) {
						$blog_id = $blog->userblog_id;
					}
					$checkout_url = $blog->checkout_url;
					if(is_pro_site( $blog->userblog_id )){
						// $checkout_url = $blog->siteurl.'/wp-login.php';
						$checkout_url = $blog->siteurl;
					}
					$content .= '<li><a href="' . $checkout_url . '">' . $upgrade_label . '</a><hr> (<em>' . $blog->siteurl . '</em>) - ' . $blog->level_label . '</li>';
				...
				$content .= '<h3>' . __( 'Veuillez sélectionner le site à modifier <br> ou créer un nouveau:', 'psts' ) . '</h3>';
			}
			...
			if( !empty( $user_domain ) && $allow_multi ) {
				...
				$content .= '<div class="psts-signup-another">' . $inactive_site . '<a href="' . esc_url( $this->checkout_url() . '?action=new_blog' ) . '">' . esc_html__( 'Créer un nouveau site.', 'psts' ) . '</a>' . '</div>';
			}elseif( empty( $user_domain ) ) {
				//Don't have a site, let user create one
				$content .= '<div class="psts-signup"><a href="' . esc_url( $this->checkout_url() . '?action=new_blog' ) . '">' . esc_html__( 'Create your own site.', 'psts' ) . '</a>' . '</div>';
...
*@param $blog_id
	...
	* @param $meta
	...
	function process_manual_signup( $blog_id, $user_id, $password, $signup_title, $meta ) {
		...
		// wp_set_password( $password, $user_id ); -> Commented out wg Passwort Änderungen - HB 20.6.2018
	}
