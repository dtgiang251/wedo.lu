				...
				if ( ( ! empty( $user_check->errors ) && 1 == count( $user_check->errors ) && ! isset( $_POST['new_blog'] ) ) || ( 0 == count( $user_check->errors ) && isset( $_POST['new_blog'] ) ) ) {
					...
					if ( $keys && ! in_array( 'availability_check_only', $keys ) && ! isset( $_POST['new_blog'] ) ) {
						...
						if ( $trial_active ) {
							...
							if ( $recurring ) {
								// $blog_data['new_blog_details']['reserved_message'] = sprintf( '<div class="reserved_msg"><h2>' . __( 'Activation de votre site', 'psts' ) . '</h2>' . __( "<p>Votre site web <strong>(%s)</strong> a été réservé mais n'est pas encore activé</p><p>Veuillez s'il vous plait cliquer sur 'Activer' pour lancer la demande d'activation. Nous allons vous répondre par email, dès que l'activation sera terminée</p>", 'psts' ) . '</div>', $site_name );
								$blog_data['new_blog_details']['reserved_message'] = sprintf( '<div class="reserved_msg"><h2>' . __( 'Activation of your website', 'psts' ) . '</h2>' . __( "<p>Your website  <strong>(%s)</strong> has been reserved but not yet activated</p><p>Please click on 'Activate' to start the activation request. We will reply to you by email as soon as the activation is complete</p>", 'psts' ) . '</div>', $site_name ); 
							} else {
