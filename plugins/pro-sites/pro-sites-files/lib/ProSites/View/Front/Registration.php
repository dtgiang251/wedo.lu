	...
	class ProSites_View_Front_Registration {
		...
		public static function render_signup_form( $render_data = array(), $errors = false ) {
			...
			if ( 'sign_up' == $action || 'another_blog' == $action ) {
				...
				$content .= '<h2>' . esc_html__( 'Configurez votre site web', 'psts' ) . '</h2>';
				...
				$content .= '<div><input type="button" id="check-prosite-blog" value="' . esc_attr__( 'Réservez votre site', 'psts' ) . '" /></div>';
...
//			if ( !is_subdomain_install() ) {
			$content .= '<div class="blogname"><label for="blogname">' . __('Adresse URL de votre site: ', 'psts') . '</label>';
//			} else {
...
//			}
			...
			$content .= '<div class="blog_title"><label for="blog_title">' . esc_html__('Titre de votre site', 'psts' ) . '</label>';
