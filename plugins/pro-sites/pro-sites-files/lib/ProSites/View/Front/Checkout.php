	...
	class ProSites_View_Front_Checkout {
				...
				if ( $show_feature_table ) {
					...
					foreach ( $column['features'] as $index => $feature ) {
						...
						if ( ! empty( $feature['text'] ) ) {
							$content .= '<div class="feature-text">' . __( ProSites::filter_html( $feature['text'] ) , 'psts' ) . '</div>';
						}
				...
				if ( $show_header ) {
					...
					foreach ( $pricing_levels_order as $level ) {
						if ( ! self::$new_signup ) {
							...
							$columns[ $col_count ]['button'] = '<button class="choose-plan-button" '.$disable.'>' . __( 'Sélectionner', 'psts' ) . '</button>';
						} else {
		...
		public static function render_tables_wrapper( $section, $echo = false ) {
			...
			switch ( $section ) {
				...
				case 'pre':
					$content .= '<div id="prosites-checkout-table" data-period="' . $period . '" data-level="' . $level . '" data-lang="'. ICL_LANGUAGE_CODE .'">';
					break;
