...
function setup_enroviment() {
	...
	BX_User::add_role();
	global $box_general, $box_currency, $app_api, $checkout_mode, $escrow, $symbol;
...
function bx_pre_get_filter( $query ) {
    ...
    if ( $query->is_main_query() && is_post_type_archive( PROJECT ) && !is_admin()  || ( $post_type == PROJECT  && $action == 'sync_search' )   )  {
		...
		if ( is_user_logged_in()):
			...
			$role = ( array ) $user1->roles;
            if (!empty($role)):
		    if ( !in_array( 'freelancer', (array) $user1->roles ) ) {
    ...
    if (  $query->is_main_query() && is_post_type_archive( PROFILE ) && ! is_admin() || ( $post_type == PROFILE  && $action == 'sync_search' )    )  {
    	...
    	//$query->set( 'meta_key', 'is_available');
...
function boxtheme_scripts() {
	...
	if( BOX_CDN ){
		wp_enqueue_style( 'bootraps', 'https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css' , array( 'boxtheme-style' ), BX_VERSION );
	} else {
		wp_enqueue_style( 'bootraps', get_template_directory_uri() .'/library/bootstrap/css/bootstrap.css' , array( 'boxtheme-style' ), BX_VERSION );
	}
	...
	if ( is_customize_preview() ) {
		wp_enqueue_style( 'boxtheme-ie9', get_template_directory_uri(). '/assets/css/ie9.css' , array( 'boxtheme-style' ), BX_VERSION);
	...
	if ( is_singular() ) {
		...
		if( is_singular( PROJECT ) ) {
			...
			wp_enqueue_style( 'single-project', get_template_directory_uri(). '/assets/css/single-project.css' , array( 'boxtheme-style' ), BX_VERSION );
			wp_enqueue_script( 'single-project-js', get_template_directory_uri(). '/assets/js/single-project.js' , array( 'front' ), BX_VERSION, true );
	...
	if( is_page_template( 'page-post-project.php') || is_post_type_archive(PROJECT) || is_post_type_archive(PROFILE) ){

		wp_enqueue_style( 'post-project', get_template_directory_uri(). '/assets/css/post-project.css' , array( 'boxtheme-style' ), rand() );
		...
		if( is_page_template( 'page-post-project.php') ) {
			wp_enqueue_script( 'post-project1', get_template_directory_uri(). '/assets/js/post-project.js', array( 'jquery','chosen-js','plupload', 'define' ), BX_VERSION, true );
			$translation_array1 = array(
				'description_text' => __( 'Vous devez fournir une description.', 'box-theme' ),
				);
				wp_localize_script( 'post-project1', 'object_name1', $translation_array1 );
		}
