...
Class BX_Profile extends BX_Post{
	...
	function get_meta_fields(){
		return array('professional_title', 'address', 'hour_rate','phone_number', 'video_url','is_available','is_subscriber');
	}
	...
	function update($args){
		...
		if( !is_wp_error( $id )){
			...
			if( isset($args['secondary-notification-email']) ){
				update_user_meta( $user_ID, 'secondary_notification_email', $args['secondary-notification-email'] );
			}
	...
	function update_profile_status($args){
		...
		if ( $p->post_author == $user_ID || is_current_box_administrator() ){
			$status = isset( $args['is_available'] ) ? $args['is_available'] :'';
			if( $status == 'on' )
				wp_publish_post($profile_id);
			else  if ($status == 'off'){
				wp_update_post( array('ID' => $profile_id,'post_status' => 'inactive') );
			}
