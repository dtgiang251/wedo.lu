...
class BX_AJAX {
	...
	public static function bx_signup(){
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			Box_ActMail::get_instance()->act_signup_mail($user_data, $mail_to);
	...
	public static function sync_project(){
		...
		if ( !is_wp_error( $return ) ) {
			...
			$job_lik = home_url('/project-success/'); 
			$pay_link = add_query_arg( 'category',$request['request']['project_cat'], $job_lik);
	...
	*/
	...
	static function sync_attachment(){
		...
		if( ! empty( $project_id) ) {
			...
			if ( in_array($project->status, array('awared','done','disputing','resolved') ) &&
