...
if ( ! defined( 'ABSPATH' ) ) exit;
	class BX_Post {
		...
		function run_after_insert($post_id){
		...
		function update_post_taxonomies( $post_id, $args ){
			...
			if( $post_obj->post_type == 'profile' ) {
					...
					if( isset( $args[$tax]) ){
						...
						foreach( $languages as $lang ) {
							...
							if( is_array($args[$tax]) ) {
								foreach( $args[$tax] as $term_id ) {
									$term = icl_object_id( $term_id, $tax, $lang['code'] );
									if( $tax == 'skill' ) {
										// var_dump( $term );
									}
									if( $term ){
										...
										$terms_array[] = absint( $term->term_id );
									}
								}
							}
							else {
								$term = icl_object_id( $args[$tax], $tax, $lang['code'] );
								if( $term ){
									$term = absint( $term );
									$term = get_term( $term, $tax );
									if( $term ) $terms_array[] = absint( $term->term_id );
								}
							}
						...
						$t = wp_set_post_terms($post_id, $terms_array, $tax);
						// exit;
					}
				}
			}
				...
				foreach ($taxonomies as $tax) {
					...
					if( isset( $args[$tax] ) ){
						if( is_array( $args[$tax] ) || is_numeric( $args[$tax] ) ) {
							
							if( is_numeric( $args[$tax] ) ) {
								$args[$tax] = array( absint( $args[$tax] ) );
								$t = wp_set_post_terms($post_id, $args[$tax], $tax);
							}
							if( is_array( $args[$tax] ) ) {
								$skills = array();
								foreach( $args[$tax] as $tax_id ) {
									$skills[] = absint( $tax_id );
									$t = wp_set_post_terms($post_id, $skills, $tax);
								}
							}
						}
						else {
							$term = get_term_by( 'slug', $args[$tax], $tax );
							if( $term ) {
								$t = wp_set_post_terms($post_id, array( $term->term_id ), $tax);
							}
						}
