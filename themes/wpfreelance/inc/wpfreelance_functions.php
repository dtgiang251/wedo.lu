...
function box_header_link_in_dashboard(){
	...
	$my_bidding = add_query_arg('section','my-bidding',$dashboard_link);
	?>
	...
	<div class="container">
		<div class="row">
			<ul class="inline">
				...
				<li><a href="<?php echo $my_bidding;?>"><?php _e('My Bidding','boxtheme');?></li>
...
function progress_after_verify_success($user_data){
	...
	if( $role == FREELANCER ) {
		...
		//$profile_status = BOX_VERIFICATION ? 'inactive' : 'publish';
		...
		if( empty($profile_id ) || ! $profile_id ){
			...
			if( BOX_VERIFICATION ){
				$metas['is_admin_reviewed'] = 'default';
			}
