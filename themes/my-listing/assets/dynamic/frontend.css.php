<?php
    ...
    $color_regex = join( '', [
        '/^(',
            ...
            '|hsl\(\s*((\d{1,2}|[1-2]\d{2}|3([0-5]\d|60)))\s*,\s*((\d{1,2}|100)\s*%)\s*,\s*((\d{1,2}|100)\s*%)\)',
        ')$/',
    ] );

	  $brand = ! empty( $_GET['brand'] ) ? $_GET['brand'] : '#f24286';
    if ( ! preg_match( $color_regex, $brand ) ) {
        $brand = '#f24286';
    }
?>
...
.profile-header .profile-menu ul li.active a { border-color: <?php echo $brand; ?>; }
...
.pricing-item.featured .select-plan { background: <?php echo $brand; ?>; }
...
.bl-tabs .nav-tabs>li.active>a, .finder-tabs .nav-tabs>li.active>a:focus, .finder-tabs .nav-tabs>li.active>a:hover { border-color: <?php echo $brand; ?>; color: <?php echo $brand; ?>; }
...
.finder-tabs .nav-tabs>li.active>a:hover { border-color: <?php echo $brand; ?>; color: <?php echo $brand; ?>; }
...
.single-comment .listing-rating { background: <?php echo $brand; ?>; }
...
.woocommerce-MyAccount-navigation ul li.is-active a { background: <?php echo $brand; ?>; }
...
.woocommerce div.product span.onsale { background: <?php echo $brand; ?>; border-color: <?php echo $brand; ?>; }
ul.products li.product .onsale,
.woocommerce ul.products li.product .onsale { background: <?php echo $brand; ?>; border-color: <?php echo $brand; ?>; }
...
.woocommerce-MyAccount-navigation ul li a:hover { color: <?php echo $brand; ?>; }
.woocommerce-MyAccount-navigation ul li.current-menu-item a { background: <?php echo $brand; ?>; }
.woocommerce-MyAccount-navigation ul li.is-active a { background: <?php echo $brand; ?>; }
...
@media (min-width:1201px) {
  .elementor-widget-container .job_listing_preview_title { background: <?php echo $brand; ?>; }
...
$loaderColor = ! empty( $_GET['loader'] ) ? $_GET['loader'] : $defaultColor;

// Validate spinner color.
if ( array_keys( $loaderColor ) !== ['r', 'g', 'b', 'a'] ) {
    $loaderColor = $defaultColor;
}

$loaderColor['r'] = intval( $loaderColor['r'] );
$loaderColor['g'] = intval( $loaderColor['g'] );
$loaderColor['b'] = intval( $loaderColor['b'] );
$loaderColor['a'] = floatval( $loaderColor['a'] );

if ( min( $loaderColor ) < 0 || max( [ $loaderColor['r'], $loaderColor['g'], $loaderColor['b'] ] ) > 255 || $loaderColor['a'] > 1 ) {
  $loaderColor = $defaultColor;
}
