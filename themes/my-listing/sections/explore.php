...
<?php if (!$data['template'] || $data['template'] == 'explore-1' || $data['template'] == 'explore-2'): ?>
	<?php $data['listing-wrap'] = 'col-md-12 grid-item'; ?>
...
<?php if ($data['template'] == 'explore-no-map'): ?>
	<?php $data['listing-wrap'] = 'col-md-4 col-sm-6 reveal grid-item'; ?>
			...
			<div class="container">
				<div class="fl-head row reveal">
					...
					<div class="col-md-4 col-sm-6 col-xs-6">
						<span class="fl-results-no" :style="state.activeTab == 'search-form' || state.activeTab == 'listing-types' ? '' : 'text-align: right;'" v-cloak>
							<i class="mi remove_red_eye" v-show="state.activeTab != 'search-form' && state.activeTab != 'listing-types'"></i>
							<span></span>
						</span>
					</div>

					<div class="col-md-4 col-sm-6 col-xs-6 sort-results" v-show="state.activeTab == 'search-form' || state.activeTab == 'listing-types'" v-cloak>
						<?php foreach ( $explore->store['listing_types'] as $type ): ?>
							<?php require locate_template('partials/facets/order.php') ?>
						<?php endforeach ?>
					</div>
