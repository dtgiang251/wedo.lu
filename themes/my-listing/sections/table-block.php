		...
		<div class="pf-body">
			<ul class="extra-details">
				<?php foreach ((array) $data['rows'] as $row):
					...
					if( $row['title'] == 'Site web:' ) {
						$before = '<a target="_blank" href="'. esc_url($row['content']) .'">';
						$after = '</a>';
					}
					if( $row['title'] == 'Email:' ) {
						$before = '<a href="mailto:'. $row['content'] .'">';
						$after = '</a>';
					}
					...
					<li>
						...
						<div class="item-property"><?php echo $before . $row['content'] . $after; ?></div>
					</li>
				<?php endforeach ?>
			</ul>
		</div>
