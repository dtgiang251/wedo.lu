	...
	public function handle() {
		...
		if ( $listings->have_posts() ) : $result['found_jobs'] = true;
			...
			// while ( $listings->have_posts() ) : $listings->the_post();
				...
				// if( isset($args['search_keywords']) && strtolower(get_the_title()) == strtolower($args['search_keywords']) ) {
					// $top[] = get_the_ID();
					// global $post; $post->c27_options__wrap_in = $listing_wrap; $post->_c27_show_promoted_badge = false;
					// get_job_manager_template_part( 'content', 'job_listing' );
					// // var_dump( get_post_meta($post->ID, 'gi_premium_service', true) );
					// $result['data'][] = $post->_c27_marker_data;
				// }
			// endwhile;
			...
			while ( $listings->have_posts() ) : $listings->the_post();
				...
				// // if( !in_array( get_the_ID(), $top ) ) {
					
				// delete_post_meta( $post->ID, '_listing_preview_cache' );
				
				global $post; $post->c27_options__wrap_in = $listing_wrap; $post->_c27_show_promoted_badge = false;
				// if( $html_cache = get_post_meta( $post->ID, '_listing_preview_cache', true ) ) {
					// // $listings_html .= get_post_meta($post->ID, 'gi_premium_service', true);
					// $listings_html .= $html_cache;
				// }
				// else {
					ob_start();
					get_job_manager_template_part( 'content', 'job_listing' ); 
					$html_cache = ob_get_contents();
					// update_post_meta( $post->ID, '_listing_preview_cache', $html_cache );
				...
				// }
				if( isset($args['search_keywords']) && strtolower(get_the_title()) == strtolower($args['search_keywords']) ) {
					array_unshift( $result['data'], $post->_c27_marker_data );
				}	
				else {
					$result['data'][] = $post->_c27_marker_data;
				}	
