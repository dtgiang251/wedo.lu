...
class Query {
	...
	public function output( $query, $args = [] ) {
		...
		$term_id = isset( $_REQUEST['term'] ) ? $_REQUEST['term'] : 0;
		...
		if( $term_id && $taxonomy ) {
			...
			if( $term ) {
				...
				foreach ($all_languages as $lang => $row) {
					$term_id_lang = icl_object_id( $term->term_id, $taxonomy, false, $lang );
					// var_dump( $term_id_lang, $term->term_id, $lang );
					if( $term_id_lang == $term_id ) {
		...
		if ( $query->found_posts < 1 ) {
			...
			if( $term_name ) {
				$result['showing'] = sprintf( __( 'Wedo.lu has found one company for %s', 'my-listing' ), $term_name );
			}
			else {
			...
			if( $term_name ) {
				$result['showing'] = sprintf( __( 'Wedo.lu has found %d companies for %s', 'my-listing' ), $query->found_posts, $term_name );
			}
			else {
