...
class WP_Job_Manager {
	...
	public function choose_package() {
		...
		$all_language_package_ids = array();
		$package_ids[] = 60887;
		foreach($package_ids as $package_id){
			$current_language_id = $package_id;
			
			$all_language_package_ids[] = apply_filters( 'wpml_object_id', $current_language_id, 'product', FALSE, 'de' );
			$all_language_package_ids[] = apply_filters( 'wpml_object_id', $current_language_id, 'product', FALSE, 'en' );
			$all_language_package_ids[] = apply_filters( 'wpml_object_id', $current_language_id, 'product', FALSE, 'fr' );
		}
		
		$user_packages = self::get_user_packages( get_current_user_id(), $all_language_package_ids );
	...
	public function process_package_handler() {
		...
		if ( $is_user_package ) {
			...
			if ( $product->get_price() == 0 && apply_filters( 'mylisting\packages\free\skip-checkout', true ) === true && $product->get_meta( '_disable_repeat_purchase' ) !== 'yes' ) {
