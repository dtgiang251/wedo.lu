 ...
 */
function case27_paid_listing_claim_approve_claim( $claim_id, $send_email = true ) {
	...
	$listing_id_fr = apply_filters( 'wpml_object_id', absint( $claim->_listing_id ), 'job_listing', TRUE ,'fr' );
    $listing_id_en = apply_filters( 'wpml_object_id', absint( $claim->_listing_id ), 'job_listing', TRUE ,'en' );
    $listing_id_de = apply_filters( 'wpml_object_id', absint( $claim->_listing_id ), 'job_listing', TRUE ,'de' );
	update_post_meta( absint( $claim->_listing_id ), '_claimed', 1 );
	update_post_meta( $listing_id_fr, '_claimed', 1 );
	update_post_meta( $listing_id_en, '_claimed', 1 );
	update_post_meta( $listing_id_de, '_claimed', 1 );
	...
	if ( $claim->_user_id ) {
		...
		wp_update_post( array(
			'ID'            => absint( $listing_id_fr ),
			'post_author'   => absint( $claim->_user_id ),
		) );
		wp_update_post( array(
			'ID'            => absint( $listing_id_en ),
			'post_author'   => absint( $claim->_user_id ),
		) );
		wp_update_post( array(
			'ID'            => absint( $listing_id_de ),
			'post_author'   => absint( $claim->_user_id ),
		) );
	}
...
function case27_paid_listing_claim_info( $claim_id, $listing_id ) {
    ...
    <div class="container">
        <h3 class="success-icon">Nous avons bien reçu votre proclamation qui est en attente de validation</h3>
        <div class="">
        ...
        <a href="<?php echo home_url();?>" class="buttons button-2">OK</a>
    </div>
    </div>
...
function case27_paid_listing_claim_send_claim_email( $claim_id ) {
	...
	$args = array(
		'to'          => '%claimer_email%',
		'subject'     => __( 'Votre demande de "%listing_title%" est approuvée.', 'my-listing' ),
		'message'     => __(
			'Bonjour %claimer_name%,' . "\n" .
			"Voici le détail de votre proclamation:" . "\n\n" .
			'Listing URL: %listing_url%' . "\n" .
			'Proclamation faite par: %claimer_name%' . "\n" .
			'Etat de la proclamation: %claim_status%' . "\n\n" .
			'Après une vérification de vos données, nous allons valider votre proclamation et vous aurez accès à votre fiche entreprise.' . "\n\n" .
			'Très cordialement,' . "\n\n" .
			'wedo.lu' . "\n" .
			'Rudolphe Aben' . "\n" .
			'T: 42451144' . "\n" .
