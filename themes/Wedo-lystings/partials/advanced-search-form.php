		...
		<div id="select-listing-type" class="listing-type-select-tab c27-explore-categories search-tab tab-pane fade" :class="state.activeTab == 'listing-types' ? 'in active' : ''">
			...
			<?php foreach ( $explore->store['listing_types'] as $listing_type ): ?>
				...
				<div class="listing-cat type-<?php echo esc_attr( $listing_type->get_slug() ) ?>"
					...
					<a @click.prevent="state.activeListingType = <?php echo c27()->encode_attr( $listing_type->get_slug() ) ?>; state.activeListingTypeData.name = <?php echo c27()->encode_attr( $listing_type->get_plural_name() ) ?>; state.activeListingTypeData.icon = '<?php echo esc_attr( $listing_type->get_setting( 'icon' ) ) ?>'; state.activeTab = 'search-form'; _getListings();">
						...
						<div class="lc-info">
							<p><?php echo esc_html( $listing_type->get_plural_name() ) ?></p>
					...
					<?php foreach ($term_group as $term):
						...
						<div class="listing-cat" :class="<?php echo esc_attr( $term->get_id() ) ?> == taxonomies.categories.term ? 'active' : ''">
							<a @click.prevent="taxonomies.categories.term = '<?php echo esc_attr( $term->get_id() ) ?>'; taxonomies.categories.page = 0; getListingsByTaxonomy();">
								...
								<div class="lc-info">
									<p class="case27-secondary-text"><?php echo esc_html( $term->get_name() ) ?></p>
									<p class="count"><?php echo esc_html( $term->get_count() ) ?></p>
								</div>
								<div class="lc-icon">
		...
		<div class="listing-regions-tab tab-pabe fade c27-explore-regions" :class="state.activeTab == 'regions' ? 'in active' : ''">
			<?php if ( $explore->active_region ): ?>
				<div class="searching-for">
					...
					<?php printf( '<p class="searching-for-text">' . __( 'Searching for listings in %s', 'my-listing' ) . '</p>', '</p><span class="filter-label">' . $explore->active_region->get_name() . '</span><p>' ) ?>
				</div>
			<?php endif ?>
		</div>
		...
		<div class="listing-tags-tab tab-pabe fade c27-explore-tags" :class="state.activeTab == 'tags' ? 'in active' : ''">
			<?php if ( $explore->active_tag ): ?>
				<div class="searching-for">
					...
					<?php printf( '<p class="searching-for-text">' . __( 'Searching listings for %s', 'my-listing' ) . '</p>', '</p><span class="filter-label">' . $explore->active_tag->get_name() . '</span><p>' ) ?>
				</div>
			<?php endif ?>
		</div>
	</div>
