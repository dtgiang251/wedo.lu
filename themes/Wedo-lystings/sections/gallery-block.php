				...
				<div class="gallerySlider car-slider">
					<div class="owl-carousel galleryPreview">
						<?php foreach ((array) $gallery_items as $item): ?>
							<?php if ($item['image']['url']): ?>
								<?php
									$media_id = giang_get_image_id($item['image']['url']); ;
									$alt = 'image';
									if($media_id) $alt = giang_get_media_alt( $media_id );
								?>
								<a class="item" href="<?php echo esc_url( $item['large_image'] ? $item['large_image']['url'] : $item['image']['url'] ) ?>">
								   <img alt="<?php echo $alt; ?>" src="<?php echo esc_url( $item['image']['url'] ) ?>">
								</a>
							<?php endif ?>
						<?php endforeach ?>
					</div>
