...
<?php endif; ?>
		...
		<form class="woocommerce-form woocommerce-form-login login" method="post">
			...
			<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
				<label for="username"><?php esc_html_e( 'E-Mail adress', 'woocommerce' ); ?> <span class="required">*</span></label>
...
<?php if ( get_option( 'woocommerce_enable_myaccount_registration' ) === 'yes' ) : ?>
	...
	<div class="u-column2 col-2">
		...
		<form method="post" onsubmit="return validateFormRegister()" class="register" name="register">
			...
			<div class="md-checkbox"> 
				...
				<div class="math-captcha">
					<span id="num1"><?php echo rand(1,4) ?></span> + <span id="num2"><?php echo rand(5,9) ?></span> =
					<input id="captcha" class="captcha" type="text" name="captcha" maxlength="2" />
					<div class="error"><?php _e('Incorrect value, please try again.','wedo-listing');?></div>
				</div>
...
function validateFormRegister() {	
    $result = ( parseInt( jQuery('#num1').text() ) + parseInt( jQuery('#num2').text() ) == parseInt( jQuery('#captcha').val() ) ) ;
    if ( $result == false) {
